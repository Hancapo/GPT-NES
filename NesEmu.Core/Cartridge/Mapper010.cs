...
public sealed class Mapper010 : Mapper
{
    ...
    public Mapper010(byte[] prgRom, byte[] chrMemory, byte[] prgRam, MirroringMode mirroring, bool chrWritable)
    ...
    public override void OnPpuAddressAccess(ushort address)
    {
        switch (address)
        {
            case >= 0x0FD8 and <= 0x0FDF:
                _latch0Fe = false;
                break;
            case >= 0x0FE8 and <= 0x0FEF:
                _latch0Fe = true;
                break;
