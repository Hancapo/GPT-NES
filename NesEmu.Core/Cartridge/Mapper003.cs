...
public sealed class Mapper003 : Mapper
{
    ...
    public Mapper003(byte[] prgRom, byte[] chrMemory, byte[] prgRam, MirroringMode mirroring, bool chrWritable)
    ...
    {
        return address < 0x2000 ? ReadChr8(_chrBank, address) : (byte)0xFF;
    }
    ...
    public override void PpuWrite(ushort address, byte value)
    {
        if (address < 0x2000)
        {
            WriteChr8(_chrBank, address, value);
        }
    }
}
