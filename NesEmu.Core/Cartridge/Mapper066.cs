...
public sealed class Mapper066 : Mapper
{
    private readonly MirroringMode _mirroring;
    private byte _bankRegister;

    public Mapper066(byte[] prgRom, byte[] chrMemory, byte[] prgRam, MirroringMode mirroring, bool chrWritable)
    ...
    public override byte CpuRead(ushort address)
    {
        ...
        return address < 0x8000 ? (byte)0xFF : ReadPrg32((_bankRegister >> 4) & 0x03, address - 0x8000);
    }
    ...
    {
        return address < 0x2000 ? ReadChr8(_bankRegister & 0x03, address) : (byte)0xFF;
    }
    ...
    public override void PpuWrite(ushort address, byte value)
    {
        if (address < 0x2000)
        {
            WriteChr8(_bankRegister & 0x03, address, value);
        }
    }
}
